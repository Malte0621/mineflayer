...
function inject (bot, options) {
  ...
  bot._client.on('player_chat', (packet) => {
    const message = packet.unsignedChatContent || packet.signedChatContent || packet.plainMessage
    let verified = false
    ...
    const parameters = {
      sender: JSON.parse(packet.senderName || packet.networkName),
      content: typeof message === 'string' ? message : JSON.parse(message),
    }
